...
const Mic = React.forwardRef<{ startAlwaysOnMode: () => void, stopAlwaysOnMode: () => void }, MicProps>(
  ({ onAudioChunk }, ref) => {
    ...
    const startAlwaysOnMode = useCallback(() => {
    ...
    }, [ref, startAlwaysOnMode, stopAlwaysOnMode]);

    useEffect(() => {
      const handleKeyDown = async (e: KeyboardEvent) => {
        if (alwaysOnMode) return; // Ignore key events in always-on mode
    ...
    }, [alwaysOnMode, onAudioChunk, startRecording, stopRecording]);
