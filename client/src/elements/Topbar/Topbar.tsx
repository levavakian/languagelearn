...
export const GetBuyModal = () => {
    ...
    const mobilePayButton = (amount: number, id: number) => {
        return <div key={id} className="flex flex-row space-x-4 overflow-visible mt-4">
            ...
            >
                <Icon name="profit" scale={30} />
                <div className="text-2xl pl-4 font-black text-center">{amount}</div>
                <div className="text-xl pl-4 font-semibold text-center">${amount / 100}</div>
            </div>
        </div>
    }
    ...
    const firstPageMobile = () => {
        return <div ref={contentRef}><div className="text-3xl font-bold">Buy Coins</div>
        <div className="text-indigo-dye w-[100%] pt-4">
            Thank you for your support! This is a beta website built on a beta voice API, so pricings and behavior may change often, but I hope you will find it useful.
        </div>
        <div className="flex flex-col gap-4">
            {mobilePayButton(500, 0)}
                ...
                >
                    <div className='flex flex-col'>
                        <div className="flex flex-row">
                            <Icon name="profit" scale={30} />
                            <div className="text-2xl pl-4 font-black text-center">Custom</div>
                            <div className="text-xl mt-1 pl-4 font-semibold text-center">${(isNaN(customAmount) ? 0 : customAmount / 100.0).toFixed(2)}</div>
                        </div>
                        <input 
                            type="number" 
                            placeholder="$" 
                            max={20000}
                            className="mt-[12px] min-h-[25px] min-w-[75px] rounded-lg bg-white text-center w-full [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none outline-none focus:outline-none focus:ring-0 border-none ring-0 ring-offset-0 focus:border-none focus:ring-offset-0"
                            value={customAmount}
                            onChange={(e) => {
                                const value = parseInt(e.target.value);
                                const limitedValue = Math.min(value, 20000);
    ...
    const secondPageMobile = () => {
        return <div style={{ width: dimensions.width, height: dimensions.height }}>
                <div className="text-3xl font-black">Buy More Coins</div>
                <div className = "h-[80%] w-[100%] flex flex-col justify-center items-center mt-4">
                    <div className="flex flex-col justify-center items-center">
                        <div className=" flex flex-row items-center justify-center space-x-4 overflow-visible mr-8">
                            <div className={`w-full flex flex-row items-center p-4 bg-[var(--alice-blue)] px-[60] rounded-lg shadow-[0_6px_0_var(--indigo-dye)] border-[2px] border-solid border-[--indigo-dye]`}>
                                <Icon name="profit" scale={30} />
                                <div className="text-2xl pl-4 font-black text-center">{selectedAmount}</div>
                                <div className="text-xl pl-4 font-semibold text-center">${(selectedAmount / 100.0).toFixed(2)}</div>
                            </div>
                        </div>
    ...
    if (smallScreen()) {
        return ShowMobileModal(
            ...
            <div className="w-full">
                {onSecondPage ? secondPageMobile() : firstPageMobile()}
            </div>,
