...
func SetupRoutes(api *mux.Router) {
	...
	api.HandleFunc("/lesson/{lessonId}", auth.AuthMiddleware(getLesson)).Methods("GET")
	...
	api.HandleFunc("/course/{id}/settings/notes", auth.AuthMiddleware(updateNoteNodes)).Methods("PUT")
	api.HandleFunc("/course/{id}/settings/vocab-items", auth.AuthMiddleware(updateVocabItems)).Methods("PUT")
	...
	api.HandleFunc("/lesson/{lessonId}/chat", auth.AuthMiddleware(getChatByLesson)).Methods("GET")
